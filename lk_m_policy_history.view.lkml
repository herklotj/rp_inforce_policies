...
SUM(CASE WHEN schedule_cover_start_dttm = annual_cover_start_dttm  AND '2021-02-01' <= annual_cover_start_dttm  AND annual_cover_start_dttm < TIMESTAMP(to_date(sysdate)) AND schedule_cover_start_dttm < schedule_cover_end_dttm AND cfi_ind = 0 AND broker_ind = 1 then 1 else 0 end) as broker_count_motor_fy22_to_date,
...
FROM lk_m_policy_history
  ...
  dimension: broker_count_motor_Financial_Year_22_to_date {
    type: number
    sql: ${TABLE}.broker_count_motor_fy22_to_date ;;
  }
